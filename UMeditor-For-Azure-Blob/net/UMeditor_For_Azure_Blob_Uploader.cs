...
namespace UMeditor_For_Azure_Blob
{
    ...
    public class Uploader
    {
        ...
        private const string CONTAINER = "umeditor";
        ...
        public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size)
        {
            ...
            try
            {
                ...
                if (state == "SUCCESS")
                {
                    ...
                    URL = DateTime.Now.ToString("yyyy") + "/" + DateTime.Now.ToString("MM") + "/" + DateTime.Now.ToString("dd") + "/";
                    URL += Guid.NewGuid().ToString() + "_" + originalName;

                    var blob = blobContainer.GetBlockBlobReference(URL);
                    blob.UploadFromStream(uploadFile.InputStream);
                }
            }
            catch (Exception e)
            {
